...
function downloadRobotsTxt() {
  const domain = appState.seo.sitemapDomain || "https://qimamsa.com";
  const cleanDomain = domain.endsWith('/') ? domain : domain + '/';
  const currentDate = new Date().toISOString().split('T')[0];
  
  const content = `# --------------------------------------------------------------------------
# Robots.txt for Qimam Almasar Co. | شركة قمم المسار للمقاولات العامة
# Automated SEO Optimization Engine - Last Updated: ${currentDate}
...
# Crawl-delay for non-critical search spiders
...
function downloadSitemapXml() {
  ...
  xml += `<!-- Generated by Qimam Almasar Co. SEO Engine - Last Updated: ${currentDate} -->\n`;
